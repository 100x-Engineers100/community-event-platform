...
**Tech Stack:** Next.js 14+ (App Router) + Supabase + Google OAuth + Tailwind + Vercel
...
### Table: verified_members (Phase 2 - Admin)
```sql
...
CREATE INDEX idx_verified_members_name ON verified_members(full_name);
```
...
**Verify host (Phase 2 - Admin):**
```sql
CREATE OR REPLACE FUNCTION verify_host(host_id uuid)
RETURNS TABLE(is_verified boolean, matched_cohort text) AS $$
BEGIN
  RETURN QUERY
  SELECT
    EXISTS(
      SELECT 1 FROM verified_members vm
      INNER JOIN profiles p ON p.id = host_id
      WHERE vm.email = p.email
      OR LOWER(vm.full_name) = LOWER(p.full_name)
    ) as is_verified,
    vm.cohort as matched_cohort
  FROM verified_members vm
  INNER JOIN profiles p ON p.id = host_id
  WHERE vm.email = p.email OR LOWER(vm.full_name) = LOWER(p.full_name)
  LIMIT 1;
END;
$$ LANGUAGE plpgsql;
```
...
GET  /api/admin/verify-host/[id]    # Check host verification status
```
...
- [ ] Import C1-C6 cohort member data
- [ ] Build host verification display
- [ ] Integrate verification badges in review interface
...
- Host verification system (cross-reference with cohort database)
...
**Purpose:** Verify hosts truthfully filled cohort/affiliation details during signup

**Implementation:**
- Maintain consolidated database of 100x Engineers members (C1-C6 cohorts)
- When reviewing events, admin sees verification status
- System cross-references host profile against verified member list
...
When reviewing events, admin sees:
```
Event Details
Host: John Doe
Email: john@example.com
Claimed Affiliation: Current Cohort

Verification Status:
✓ Verified Member - C5
  (Found in verified members database)

OR

✗ Not Verified
  (Not found in database - manual verification required)
```

**Benefits:**
- Maintains community authenticity
- Prevents fake affiliations
...
- Cron implementation: pg_cron or Supabase Edge Functions?
- Timezone handling: Store UTC, display local?
- Email notifications: Defer to post-MVP or include in V1?
- RLS policies: Exact implementation for each table?
...
Stored in memory:
  ...
  Splite component installation (when Next.js + shadcn ready):
